...
- ☎️ **Conversation length**: say how long a call lasts and the resident sequence follows it, because a conversation holds its own transcript. A twenty-minute contact-centre call is not a two-minute IVR interaction, and under a pinned KV policy every caller in flight holds their whole cache for the whole call. The three session presets now declare the shape their resident figure was built from, so `4,096` for the voice agent is no longer an assertion: it is a 3,100-token system prompt plus five minutes of talking, and you can change the five
