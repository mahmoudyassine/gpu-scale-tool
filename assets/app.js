...
const STUDIO_VERSION = '5.20.0', ENGINE_VERSION = 25;
...
function renderFleet(prj){
  ...
  { const co=prj.co||{}, lim=[];
    ...
    $('fmNote').innerHTML=notes.map(esc).join(' ')+
      ` GPU fill height is memory used; on a shared GPU each band is one model.`+
...
function serialize(){
  ...
  return {
    ...
    project:{active:activeUc, usecases:UC.map(u=>({id:u.id, name:u.name||null,
      isolate:!!u.isolate, supports:u.supports||[], concManual:!!u.concManual, sliceU:+u.sliceU||0,
...
function applyConfig(raw){
  ...
  if(proj && Array.isArray(proj.usecases) && proj.usecases.length){
    ...
    proj.usecases.forEach(pu=>UC.push({id:'uc'+(++ucSeq), name:pu.name||'',
      supports:Array.isArray(pu.supports)?pu.supports:[], isolate:!!pu.isolate, concManual:!!pu.concManual, sliceU:+pu.sliceU||0, f:{}}));
...
function solvePool(s){
  ...
  function planAt(tp){
    ...
    if(interactive){
      ...
      workers=Math.min(64,Math.max(1,Math.ceil(Math.max(1,Math.ceil(s.concurrent/BATCH_MAX))*tp/s.perW)));
      for(;;){
        const replicas=Math.max(1,Math.floor(workers*s.perW/tp));
        batch=Math.min(BATCH_MAX,Math.max(1,Math.ceil(s.concurrent/replicas)));
        d=eva(workers,batch);
      ...
      if(converged){
        ...
        for(let guard=0; guard<80; guard++){
          ...
          batch=Math.min(BATCH_MAX,Math.max(1,Math.ceil(s.concurrent/reps2)));
        }
        ...
        const wFloor=Math.max(1,Math.ceil(tp/s.perW));
        for(let guard=0; guard<80 && workers>wFloor; guard++){
          const w2=workers-1;
          const reps2=Math.max(1,Math.floor(w2*s.perW/tp));
          const b2=Math.min(BATCH_MAX,Math.max(1,Math.ceil(s.concurrent/reps2)));
          const d2=eva(w2,b2);
          if(d2.total>pack*d2.avail) break;
          const m2=missOf(d2); if(m2.t||m2.p) break;
          workers=w2; batch=b2; d=d2;
        }
        sloStuck = stuckList.length? [...new Set(stuckList)].join(' and ') : null;
        grewForSlo=Math.max(0, workers-w0);
      }
    } else {
