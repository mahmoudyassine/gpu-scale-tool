...
/* MIG slice as a synthetic smaller GPU (NVIDIA MIG guide): compute scales with
   u of max SM slices, bandwidth with the MEMORY-slice map m/8 (1g:1, 2g:2,
   3g:4, 4g:4, 7g:8) at 0.93 delivered efficiency; memory = profile size.
   The engine then runs unchanged on that synthetic part. */
const MIG_BW_MAP7={1:1,2:2,3:4,4:4,7:8};
function sliceProfiles(g){
  const part=g.part||{}; if(part.kind!=='mig'&&part.kind!=='cpx') return [];
  const out=[];
  for(let u=1;u<part.max;u++){
    if(part.kind==='mig'&&part.max===7&&!(u in MIG_BW_MAP7)) continue;
    const gb=Math.min(g.vram, part.min*u);
    const bwFrac=part.kind==='mig'&&part.max===7? MIG_BW_MAP7[u]/8 : u/part.max;
    out.push({units:u, max:part.max, gb, bw:g.bw*bwFrac*0.93, tflops:g.tflops*(u/part.max)});
  }
  return out;
}
  ...
  pools.forEach(p=>{
    ...
    if(prof){
      ...
      Object.assign(p.state, {gpuVram:prof.gb, gpuBw:prof.bw, gpuTflops:prof.tflops,
        gpus:p.state.workers, perW:1, multiGb:1.4});
    }
    p.d=compute(p.state);
...
function allocShared(pools, hw){
  ...
  pools.forEach((p,pi)=>{ if(!p.sliced) return;
    for(let r2=0;r2<p.d.replicas;r2++) items.push({kind:'pool', pool:pi, units:p.sliced.units,
      gb:p.sliced.gb, tip:`${p.state.model.name} replica ${r2+1} · MIG ${p.sliced.units}g slice (${fmt(p.sliced.gb)} GB)`}); });
  ...
  return {sup, bins, gpus:bins.length,
    note:`isolated ${part.kind==='mig'?'MIG':'compute'} slices (${part.max} per GPU): sliced model replicas and supporting models share physical GPUs`};
}
...
function buildProjectSummary(prj){
  ...
  pools.forEach((p,pi)=>{
    ...
    h+= p.sliced
      ? li('layers',`<b>${esc(st.model.name)} ${esc(st.wq.name)}</b>: ${p.d.replicas} replicas on MIG ${p.sliced.units}g slices (${fmt(p.sliced.gb)} GB each) serve ${p.d.active} of ${st.concurrent} pooled calls, sharing physical GPUs with other models and supports · ${(p.d.total/p.d.avail*100).toFixed(0)}% of slice memory used<span class="dtl">; each slice gets ${p.sliced.units}/${(hw.g.part||{}).max} of the GPU's compute and a hardware-isolated bandwidth share, so speed figures are per-slice honest (batch ${st.batch})</span>`)
...
function buildFleetSites(prj){
  ...
  function gpuList(){
    ...
    (shared.bins||[]).forEach(bin=>list.push({type:'shared', bin,
      tip:'shared GPU: '+bin.slices.map(sl=>sl.tip||sl.kind).join(' · ')}));
    return list;
  }
  function serveNodesAll(copy){
    ...
    for(let base=0; base<list.length || out.length===0; base+=hw.perW){
      ...
      node.util=chunk.length? chunk.reduce((x,g2)=>x+(g2.type==='pool'? g2.util : g2.type==='shared'? Math.min(1,g2.bin.used/(part.max||1)) : 0),0)/hw.perW : 0;
...
function renderFleet(prj){
  ...
  const gpuHtml=g2=>{
    ...
    if(g2.type==='sup'||g2.type==='shared'){ let y=0;
      const bands=g2.bin.slices.map(sl=>{ const h=Math.max(10, (sl.units||sl.n||1)/partMax*100);
...
function renderProjectReport(prj){
  ...
  box.innerHTML=pools.map((p,pi)=>{
    ...
    return `<div class="pl-row">
      <div class="pl-lab"><span class="dot" style="background:var(--pool${PC[pi]})"></span>${esc(tag(pi))}<span class="pl-note">${p.sliced?`${p.d.replicas}× MIG ${p.sliced.units}g`:`${p.d.replicas}×TP${p.state.tp} · ${p.state.workers} node${p.state.workers>1?'s':''}`}</span></div>
...
function solvePool(s){
  ...
  if(tp===1 && interactive){
    ...
    for(const prof of profs){
      ...
      let slices=Math.max(1,Math.ceil(s.concurrent/64)), sb=0, sd=null, okFit=false, stuck=null;
      ...
      for(let guard=0; guard<200; guard++){
        ...
        if(sb<=1 || slices>=256){ if(missT2||missP2) stuck=(missT2?'per-user speed':'')+(missT2&&(missP2)?' and ':'')+(missP2?'P95':''); break; }
        slices++;
        sb=Math.min(64,Math.max(1,Math.ceil(s.concurrent/slices)));
        sd=compute({...syn, gpus:slices, workers:slices, batch:sb});
      }
      if(stuck) continue;
      const phys=slices*prof.units/prof.max;
...
function autoSizeProject(quiet){
  ...
  prj.pools.forEach(p=>{
    ...
    lines.push(r.mode==='sliced'
      ? `${p.state.model.name} ${p.state.wq.name} (${names}): ${r.workers} replicas on MIG ${r.sliceU}g slices (${fmt(r.prof.gb)} GB each) · batch ${r.batch} for ${p.state.concurrent} pooled calls · shares physical GPUs with other models and supports (≈${(r.physGpus).toFixed(1)} GPU-equivalents).`
