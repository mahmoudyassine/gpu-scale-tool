...
const STUDIO_VERSION = '5.34.1', ENGINE_VERSION = 27;
...
function readState(){
  ...
  // A fresh auto-size plan owns exactly the CARDS it was solved for; the fleet
  // packs them onto whole nodes further down. Same rule as ucState, so the
  // single-use-case report describes the plan the solver actually verified.
  const u0 = UC[activeUc];
  const gpus = Math.max(1, (u0 && +u0.cards>0 && u0.cardsKey===cardsKey(u0,readHw()))? +u0.cards : workers*perW);
...
function buildStory(s,d,m,g){
  ...
  const info=RESIL[s.resil];
  {
    ...
    const mult=F? F.resilW-info.add : info.mult(s.workers);
...
/* ================= RECOMMENDATIONS ================= */
/* Is this card the right shape for the job? Decode is bandwidth bound, so a
   card can only ever hold (bandwidth x ic x mbu) / target tok/s of memory at
   the operating point. When that ceiling sits far under the card's VRAM you
   are buying HBM the speed targets forbid you from filling, and a different
   part serves the same load on fewer cards or less power. We answer it by
   re-solving the whole project on each candidate rather than by rule of thumb. */
...
/* ================= SLO OPTIMISER ================= */
/* The speed target is a sizing lever, not just a promise. At the bandwidth
   limited operating point a card can only hold
       memory per card = gpuBw x ic x mbu / target tok/s
   GB, whatever the model and whatever the TP width. Invert it and the target
   that fills a fraction U of the card's VRAM is
       target tok/s = gpuBw x ic x mbu / (U x vram)
   which on a 288 GB / 8 TB/s part at U=0.8 is about 19 tok/s. A project asking
   for 60 therefore buys three times the cards to leave two thirds of every card
   empty. We suggest the target, never impose it: each use case is measured
...
/* One delegated handler: write the targets across every use case in the action's
...
{ const rp=$('recs');
  if(rp) rp.addEventListener('click',e=>{ const b=e.target.closest&&e.target.closest('[data-apply]');
    if(!b) return;
...
function render(){
  ...
  $('gpuMeta').innerHTML =
    ...
    `<div class="mrow"><span>Fleet</span><b>${s.gpus===s.workers*s.perW
      ? `${s.workers}× worker · ${s.perW} GPU = ${s.gpus} GPU`
      : `${s.gpus} GPU · packed onto shared nodes (${s.perW}/node)`}</b></div>`+
  ...
  if(UC.length===1 && s.tp===1 && (s.gpu.part||{}).kind==='mig'){
    const cw=s.params*s.bytesW + d.act;
    if(cw < 0.45*s.gpuVram) ins.push(['ok',`One ${fmt(cw)} GB copy uses under half of a ${fmt(s.gpuVram)} GB GPU. In a multi-use-case project, Auto-size can place it on a MIG slice and share the physical GPU with other models and supporting services.`]);
  }
  ...
  if(prj){
    ...
    $('vKv').innerHTML = `<b>${fmt(F.vramNeed)} GB</b> required · <b>${fmt(F.vramAvail)} GB</b> across ${F.activeG} active GPUs${F.spare?` · ${F.spare} spare`:''}`;
  ...
  $('story').innerHTML = (window.__prj && UC.length>1)? buildProjectSummary(window.__prj)
    : buildStory(s,d,m,g)+(window.__autoNote?`<div class="story-auto">${rico('zap')}<b>Sizing decision</b> (Auto-size, ${esc(window.__autoNote.at.toLocaleTimeString())}):${autoNoteHtml()}</div>`:'');
  ...
  $('printConfig').textContent = window.__prj&&UC.length>1
    ? `GPUscale.net · ${projName()} · ${UC.length} use cases in ${window.__prj.pools.length} pool(s) on ${g.name} · `+
      window.__prj.pools.map(p=>`${p.state.model.name} ${p.state.wq.name} ${p.sliced?`${p.d.replicas}×${p.sliced.kind==='mig'?'MIG '+p.sliced.units+'g':'partition'}`:`TP${p.state.tp}×${p.state.workers}w`}`).join(' + ')+
...
function duplicateUc(i){ captureUc(); const base=UC[i]; if(!base) return;
  const u={id:'uc'+(++ucSeq), name:(ucName(base)+' copy').slice(0,40),
    f:Object.assign({},base.f), supports:(base.supports||[]).map(sp=>Object.assign({},sp)), isolate:!!base.isolate, sliceU:+base.sliceU||0};
...
/* MIG slice as a synthetic smaller GPU (NVIDIA MIG guide): compute scales with
   u of max SM slices, bandwidth and memory with the MEMORY-slice map (7-slice
   parts: 1g:1, 2g:2, 3g:4, 4g:4 of 8 memory slices; 4-slice parts: 1g:1, 2g:2
   of 4) at 0.93 delivered bandwidth efficiency; only real profile geometries
   are emitted. The engine then runs unchanged on that synthetic part. */
const MIG_MEM_MAP7={1:1,2:2,3:4,4:4};   // memory slices (of 8) per compute-slice count
const MIG_MEM_MAP4={1:1,2:2};            // 4-slice parts: only 1g and 2g sub-profiles exist
...
function buildProjectSummary(prj){
  ...
  pools.forEach((p,pi)=>{
    ...
    h+= p.sliced
      ? li('layers',`<b>${esc(st.model.name)} ${esc(st.wq.name)}</b>: ${p.d.replicas} replicas on ${sliceName(p.sliced,true)} (${fmt(p.sliced.gb)} GB each) serve ${p.d.active} of ${st.concurrent} pooled calls, sharing physical GPUs with other models and supports · ${(p.d.total/p.d.avail*100).toFixed(0)}% of slice memory used<span class="dtl">; each ${p.sliced.kind==='cpx'?'partition':'slice'} gets ${p.sliced.units}/${(hw.g.part||{}).max} of the GPU's compute and a hardware-isolated bandwidth share, so speed figures are per-slice honest (batch ${st.batch})</span>`)
      : li('layers',`<b>${esc(st.model.name)} ${esc(st.wq.name)}</b>: serves ${p.d.active} of ${st.concurrent} pooled calls with ${p.d.replicas*st.tp} GPUs · ${(p.d.total/p.d.avail*100).toFixed(0)}% memory used<span class="dtl">; one ${fmt(wPer)} GB copy needs TP${st.tp} (smallest slice that fits the memory target), ${p.d.replicas} replica${p.d.replicas>1?'s':''} at batch ${st.batch}</span>`);
  });
  if(sup.items.length)
    h+=li('puzzle',`<b>Supporting models</b>: ${sup.items.map(it=>`${KIND_LABEL[it.kind]||it.kind} ×${it.instances}`).join(', ')} on ${(prj.shared&&prj.shared.gpus)||sup.gpus} shared GPU${((prj.shared&&prj.shared.gpus)||sup.gpus)>1?'s':''} (${esc((prj.shared&&prj.shared.note)||sup.note)})`);
  ...
  if(window.__autoNote)
    h+=li('zap',`<b>Sizing decision</b> (Auto-size, ${esc(window.__autoNote.at.toLocaleTimeString())}):${autoNoteHtml()}`);
  const finds=[];
  ...
  pools.forEach(p=>{ if(p.capped) finds.push(['warn',`${esc(p.state.model.name)}: ${p.capped-p.d.replicas} replica${p.capped-p.d.replicas>1?'s':''} beyond demand were trimmed, so their GPUs are not procured.`]); });
...
function buildFleetSites(prj){
  const {pools, shared, hw}=prj;
  const r=hw.resil, info=RESIL[r]||RESIL.n;
  const N=prj.fleet.servW;
  let nn=0, sp=0, idl=0;
  ...
  function gpuList(){
    ...
    // what each card actually holds, split the way the ledger splits it
    const mixOf=slots=>{ const m={w:0,kv:0,o:0};
      slots.forEach(s=>{ const p2=pools[s.pool], n=Math.max(1,p2.d.replicas*Math.max(1,p2.state.tp));
        m.w += p2.d.weightsAll/n; m.kv += p2.d.kvTotal/n;
        m.o += (p2.d.actAll+p2.d.fixed+p2.d.multi)/n; });
      return m; };
    ((prj.co&&prj.co.gpus)||[]).forEach(g=>{
      const tenants=[...new Set(g.slots.map(s=>s.pool))];
      const util=Math.min(1, g.mem/Math.max(1,hw.g.vram));
      const mix=mixOf(g.slots);
      if(tenants.length<=1){ const s0=g.slots[0]; if(!s0) return; const st=pools[s0.pool].state;
        list.push({type:'pool', pool:s0.pool, rep:s0.rep, util, mix,
          tip:`${st.model.name} · replica ${s0.rep+1} of ${pools[s0.pool].d.replicas} (TP${st.tp} shard)\n${fmt(mix.w)} GB weights + ${fmt(mix.kv)} GB KV cache + ${fmt(mix.o)} GB working set = ${fmt(g.mem)} GB of ${fmt(hw.g.vram)} GB (${(util*100).toFixed(0)}%)${st.tp>1?`\na TP${st.tp} copy spans ${st.tp} cards over NVLink`:''}`});
        return; }
      // one band per tenant, in its own pool colour: a flat teal card said
      // "all KV" and matched no legend entry, and the tooltip was the only way
      // to learn which models shared the card (gone in print).
      const slices=tenants.map(pi=>{ const st=pools[pi].state;
        const mine=g.slots.filter(s=>s.pool===pi);
        const mem=mine.reduce((t,s)=>t+s.mem,0);
        return {kind:'pool', pool:pi, gb:mem, mu:mem, mix:mixOf(mine),
          tip:`${st.model.name} (TP${st.tp}) · ${fmt(mem)} GB`}; });
      list.push({type:'shared', mix, util, bin:{co:true, used:g.mem, slices},
        tip:`co-resident models on one GPU: ${slices.map(s=>s.tip).join(' · ')}\n${fmt(mix.w)} GB weights + ${fmt(mix.kv)} GB KV cache + ${fmt(mix.o)} GB working set = ${fmt(g.mem)} GB of ${fmt(hw.g.vram)} GB (${(util*100).toFixed(0)}%)`}); });
    // Every cell reports the same metric: memory. Node headers used to average
    // MIG slice occupancy for these bins while the cell printed memory, so two
    // bare percentages 24 points apart sat next to each other.
    (shared.bins||[]).forEach(bin=>{
      const memUsed=bin.slices.reduce((t,sl)=>t+(sl.fill!=null? sl.fill*(sl.gb||0)
        : (sl.use!=null? sl.use : (sl.gb||0))),0);
      const util=Math.min(1, memUsed/Math.max(1,hw.g.vram));
      const part=hw.g.part||{max:1};
      list.push({type:'shared', bin, util, memUsed,
        tip:'shared GPU: '+bin.slices.map(sl=>sl.tip||`${KIND_LABEL[sl.kind]||sl.kind}${sl.model?' '+sl.model:''}${sl.inst>1?' ×'+sl.inst:''} (${fmt(sl.gb)} GB)`).join(' · ')
          +`\n${fmt(memUsed)} GB of ${fmt(hw.g.vram)} GB in use (${(util*100).toFixed(0)}%)`
          +(part.max>1? ` · ${bin.used} of ${binCap(part)} memory slices allocated`:'')}); });
    return list;
  }
  function serveNodesAll(copy){
    const list=gpuList(); const out=[];
    for(let base=0; base<list.length || out.length===0; base+=hw.perW){
      const node={label:(copy?'B-':'ND-')+String(++nn).padStart(2,'0'), cls:'serve', pool:null,
        poolName:'', gpus:[], active:false};
      const chunk=list.slice(base, base+hw.perW);
      // a serving pool counts whether it owns the card or co-tenants it: a node
      // of merged serving cards used to read "shared / supporting", exactly like
      // a node of embedding and ASR slices, and left node.pool undefined so the
      // selected-pool outline emitted var(--poolundefined)
      const poolsIn=new Set(), supIn=[];
      chunk.forEach(g2=>{
        if(g2.type==='pool') poolsIn.add(g2.pool);
        else if(g2.type==='shared') (g2.bin.slices||[]).forEach(sl=>{
          if(sl.kind==='pool') poolsIn.add(sl.pool); else supIn.push(sl); }); });
      node.util=chunk.length? chunk.reduce((x,g2)=>x+(g2.util||0),0)/hw.perW : 0;
      ...
      node.withSup=!!supIn.length;
      if(poolsIn.size===1){ node.pool=[...poolsIn][0];
        node.poolName=pools[node.pool].state.model.name+(supIn.length?' + supporting':''); }
      else if(poolsIn.size===0&&chunk.some(g2=>g2.type==='shared')){ node.poolName='shared / supporting'; }
      else if(poolsIn.size>1){ node.poolName='mixed'; }
      node.active=chunk.some(g2=>g2.type==='pool'&&pools[g2.pool].members.includes(activeUc))
        || chunk.some(g2=>g2.type==='shared'&&g2.bin.slices.some(sl=>sl.kind==='pool'&&pools[sl.pool].members.includes(activeUc)));
      out.push(node);
      if(base+hw.perW>=list.length) break;
    }
    return out;
  }
  function idleNodes(pi, count, mode, tag){
    ...
    for(let w=0; w<count; w++){
      ...
      const tip = mode==='standby'
        ? `${node.label} · shared spare: idle node that covers a failure in any pool (hardware is uniform)`
        : `${node.label} · ${mode==='mirror'?'mirror of the serving fleet':'DR standby for the serving fleet'}`;
      for(let k=0;k<hw.perW;k++) node.gpus.push({type:'idle', mode, pool:pi, tip:tip+` · GPU ${k+1}`});
      out.push(node);
    }
    return out;
  }
  const rsvNodes=(k)=>idleNodes(null,k,'standby','RS-');
  const mirrorNodes=(k,mode,tag)=>idleNodes(null,k,mode||'mirror',tag||'MR-');
  const sites=[];
  const site=(title,icon,nodes)=>{ sites.push({title, icon, nodes}); };
  if(r==='n'||r==='n1'||r==='n2'){
    site(`Production site · N=${N}${info.add?'+'+info.add:''}`,'building',
      [...serveNodesAll(), ...rsvNodes(info.add)]);
  } else if(r==='nn'){
    site(`System A · active · N=${N}`,'building',serveNodesAll());
    sites[sites.length-1].link='in-site failover';
    site(`System B · mirror · N=${N}`,'shield', mirrorNodes(info.mult(N)));
  } else if(r==='dr'||r==='drh'){
    site(`Primary site · active · N=${N}`,'building',serveNodesAll());
    sites[sites.length-1].link='async replication';
    const cnt=info.mult(N);
    site(r==='dr'? `DR site · standby · N=${N}` : `DR site · half-size standby · ${cnt} node${cnt>1?'s':''}`,'globe', mirrorNodes(cnt,'drs','DR-'));
  } else if(r==='aas'||r==='aas1'||r==='aass'){
    const packed=serveNodesAll();
    const nA=Math.max(1,Math.ceil(packed.length/2));
    const aServe=packed.slice(0,nA), bServe=packed.slice(nA);
    const aSpare=(r==='aas1'||r==='aass')?1:0, bSpare=r==='aass'?1:0;
    site(`Site A · active · ${aServe.length} of N=${N}${aSpare?' + spare':''}`,'building',
      [...aServe, ...rsvNodes(aSpare)]);
...
function renderProject(){
  ...
  $('projStrip').innerHTML =
    pools.map((p,pi)=>`<span class="ps-chip">${rico('layers','ri')}<span class="dot" style="background:var(--pool${PC[pi]})"></span><b>${esc(short(p.state.model.name))}</b><span class="mono">${esc(p.state.wq.name)} · ${p.sliced?`${p.d.replicas} ${sliceName(p.sliced,p.d.replicas>1)} · shared GPUs`:`TP${p.state.tp} · ${p.state.workers}×${p.state.perW} GPU · ${p.d.replicas} repl`} · ${p.state.concurrent} calls${p.capped?' · '+(p.capped-p.d.replicas)+' excess repl trimmed':''}</span><span class="dot" style="background:${p.d.fits&&p.perUc.every(x=>x.d.sloAll)?'var(--teal-strong)':'var(--red)'};border-radius:50%"></span></span>`).join('')+
    ((prj.shared&&prj.shared.gpus)? `<span class="ps-chip"><span class="dot" style="background:var(--amber-border)"></span><b>Shared GPUs</b><span class="mono">${prj.shared.gpus} GPU${prj.shared.gpus>1?'s':''} carry sliced replicas + ${sup.items.reduce((t,i2)=>t+i2.instances,0)} support instances</span></span>`:'')+
...
function renderFleet(prj){
  ...
  /* One card, drawn the way the memory ledger reads it: weights are the fixed
     cost of holding the model, KV is what the conversations occupy, and the
     working set is activations plus runtime overhead. The gap above is memory
     the speed target will not let you spend. */
  const memBands=(mix, col)=>{
    if(!mix) return '';
    const V=Math.max(1,hw.g.vram), pc=x=>Math.max(0,Math.min(100, x/V*100));
    return `<div class="ms w" style="height:${pc(mix.w)}%;background:${col}"></div>`
      + `<div class="ms kv" style="height:${pc(mix.kv)}%;background:${col}"></div>`
      + `<div class="ms o" style="height:${pc(mix.o)}%"></div>`;
  };
  const memStack=(mix, col, op)=>mix? `<div class="fm-stack" style="opacity:${op==null?1:op}">${memBands(mix,col)}</div>` : '';
  /* A co-tenanted card: the same weights / KV / working-set reading, but one
     group of bands per tenant in that pool's colour, bottom-up in placement
     order, so the card matches the legend instead of one flat teal. */
  const memStackCo=slices=>`<div class="fm-stack">`
    + slices.map(sl=>memBands(sl.mix, `var(--pool${PC[sl.pool]})`)).join('')+`</div>`;
  const gpuHtml=g2=>{
    if(g2.type==='pool'){ const col=`var(--pool${PC[g2.pool]})`;
      return `<div class="fm-gpu assigned" title="${esc(g2.tip)}">${memStack(g2.mix, col, g2.rep%2?0.78:1)}${gp(g2.util)}</div>`; }
    ...
    if(g2.type==='sup'||g2.type==='shared'){ let y=0, memUsed=0, anyLab=false;
      // keep in sync with .fm-gpu / .fleet-map.dense .fm-gpu in styles.css: the
      // label test needs the real cell height, and it silently held the pre-5.21
      // 26/34 px sizes after the cells grew to 34/46
      const cellPx=dense?34:46;
      ...
      const bands=g2.bin.slices.map(sl=>{ const h=Math.max(10, (partMax>1&&!coBin)? (sl.mu||sl.units||sl.n||1)/binCap(hw.g.part)*100 : Math.min(100,(sl.gb||1)/Math.max(1,hw.g.vram)*100));
        ...
        const pct=sl.fill!=null? Math.round(sl.fill*100) : null;
        const lab=(pct!=null && h/100*cellPx>=11)? `<span class="bp">${pct}%</span>` : '';
        anyLab=anyLab||!!lab;
        ...
        y+=h; return b; }).join('');
      const used=g2.util!=null? g2.util : (partMax>1&&!coBin)? memUsed/Math.max(1,hw.g.vram) : g2.bin.used/Math.max(1,hw.g.vram);
      // a co-resident card is several models' memory, so show the same split
      if(coBin&&g2.bin.slices&&g2.bin.slices.every(sl=>sl.mix))
        return `<div class="fm-gpu assigned" title="${esc(g2.tip)}">${memStackCo(g2.bin.slices)}${gp(Math.min(1,used))}</div>`;
  ...
  const nodeHtml=n2=>{
    const role = n2.cls==='serve'? esc(n2.pool!=null? poolTag(n2.pool)+(n2.withSup?' + sup':'') : (n2.poolName||'mixed'))
    ...
    // the outline marks the SELECTED use case's pool, so colour it from that pool:
    // a mixed or co-tenanted node has no single owner and emitted var(--poolundefined)
    const acol=PC[n2.pool!=null? n2.pool : Math.max(0, pools.findIndex(p=>p.members.includes(activeUc)))];
    return `<div class="fm-node ${n2.cls==='drs'?'standby':n2.cls}${n2.active?' fm-active':''}"${n2.active?` style="--acol:var(--pool${acol})"`:''} title="${esc(n2.label+' · '+(ROLE_LABEL[n2.cls]||n2.cls)+(n2.poolName?' · '+n2.poolName:''))}">
  ...
  $('fleetList').innerHTML='<h3>Deployment, text form</h3>'+sites.map(s2=>`<h4>${esc(s2.title)}</h4><ul>`+s2.nodes.map(n2=>{
    ...
    const parts=n2.gpus.map((g2,k)=>{
      ...
      if(g2.type==='sup'||g2.type==='shared') return `GPU ${k+1}: `+g2.bin.slices.map(sl=>sl.kind==='pool'
        ? `${pools[sl.pool].state.model.name} ${g2.bin.co? 'replica (co-resident on the whole card)' : 'replica slice'}`
        : `${KIND_LABEL[sl.sub||sl.kind]||sl.kind} ${sl.model||''}${sl.inst>1?' ×'+sl.inst:''}`).join(', ');
      if(g2.type==='idle') return `GPU ${k+1}: ${role}`;
      return `GPU ${k+1}: spare`; });
    return `<li>${n2.label} (${role}): ${parts.join('; ')}</li>`; }).join('')+'</ul>').join('');
  // note
  const notes=[];
  if((prj.shared&&prj.shared.gpus)||sup.gpus) notes.push(`${esc((prj.shared&&prj.shared.note)||sup.note)}; models and supports pack onto shared GPUs before new hardware is added`+(sites.length>1?', shown in the first site (replicate per site operationally)':'')+'.');
  ...
  // that slice's own memory (frac GPUs list plain GB, no slice cap exists)
  const fs=$('fmShared');
  if(fs){ const bins=(prj.shared&&prj.shared.bins)||[];
    if(!bins.length){ fs.innerHTML=''; fs.style.display='none'; }
    else{
      const sig=b=>b.slices.map(sl=>`${sl.kind==='pool'?'P'+sl.pool:(sl.sub||sl.kind)}|${sl.units||sl.n||0}|${sl.fill!=null?Math.round(sl.fill*100):''}|${sl.inst||1}`).join('+');
      const groups=[], seen={};
      bins.forEach(b=>{ const k=sig(b); if(seen[k]!=null) groups[seen[k]].n++; else { seen[k]=groups.length; groups.push({b, n:1}); } });
      fs.style.display='';
      fs.innerHTML=`<span class="t">Shared GPUs · memory per slice</span>`+groups.map(g=>{
        const agg=[]; const key={};
        g.b.slices.forEach(sl=>{ const k=`${sl.kind==='pool'?'P'+sl.pool:(sl.sub||sl.kind)}|${sl.units||sl.n||0}|${sl.fill!=null?Math.round(sl.fill*100):''}|${sl.model||''}|${sl.inst||1}`;
          if(key[k]!=null) agg[key[k]].k2++; else { key[k]=agg.length; agg.push({sl, k2:1}); } });
        const items=agg.map(({sl,k2})=>{
          const col=sl.kind==='pool'? `var(--pool${PC[sl.pool]})` : `var(--sup${sl.sub||sl.kind})`;
          const name=sl.kind==='pool'? pools[sl.pool].state.model.name : `${KIND_LABEL[sl.sub||sl.kind]||sl.kind}${sl.model?' '+sl.model:''}`;
          const un=(sl.units||sl.n)? ` ${sl.units||sl.n}g` : '';
          const val=sl.fill!=null? `${Math.round(sl.fill*100)}% of slice${k2>1?' each':''}` : `${fmt(sl.gb||0)} GB${k2>1?' each':''}`;
          return `<span class="fs-it"><span class="sw" style="background:${col}"></span>${esc(short(name))}${un}${k2>1?` ×${k2}`:''}${sl.inst>1?` (${sl.inst}/slice)`:''} · ${val}</span>`;
        }).join('');
        return `<div class="fs-row"><span class="fs-n">${g.n>1?`${g.n}× GPU`:'GPU'}</span>${items}</div>`;
      }).join('');
    } }
...
function renderProjectReport(prj){
  ...
  box.innerHTML=pools.map((p,pi)=>{
    ...
    return `<div class="pl-row">
      <div class="pl-lab"><span class="dot" style="background:var(--pool${PC[pi]})"></span>${esc(tag(pi))}<span class="pl-note">${p.sliced?`${p.d.replicas}× ${p.sliced.kind==='cpx'?'partition':'MIG '+p.sliced.units+'g'}`:`${p.d.replicas}×TP${p.state.tp} · ${p.state.workers} node${p.state.workers>1?'s':''}`}</span></div>
  ...
  /* A global interconnect efficiency below the default is now double-counted:
     ...
     whose copy never leaves a node. Older builds wrote 0.70 here as a side
     effect and it survives every re-export, so measure what it costs. */
  try{
  ...
  // speed target lets you fill. These carry Apply buttons, so they sit above the
  // per-pool observations and below the hardware choice.
  try{ const so=sloRecHtml(prj); for(let i=so.length-1;i>=0;i--) recs.unshift(so[i]); }catch(e){}
  // Is the card itself the wrong shape for this workload? Put it first: it is
  // the most expensive decision on the page and the easiest one to get wrong.
  try{
    const rows=gpuFitScan(prj), cur=hw.g;
    if(rows&&rows.length){
      const mine=rows.find(r=>r.g.name===cur.name);
      const best=rows.filter(r=>!r.stuck);
      const alt=best.filter(r=>r.g.name!==cur.name
      ...
      if(alt.length){
        ...
        recs.unshift(`<div class="rec ${lv}"><div class="r-t">Hardware fit: ${esc(cur.name)} is `
          ...
          +`. Re-run Auto-size after switching.</div></div>`);
...
function textShare(){
  captureUc();
  const parts=[];
  const nm=($('scenarioName').value||'').trim();
  if(nm) parts.push('name='+nm);
  parts.push('gpu='+(GPUS[+$('selGpu').value||0]||{}).name);
  parts.push('perw='+(+$('inPerW').value||8));
  parts.push('resil='+$('selResil').value);
  const dflt={inMfu:0.5, inMbu:0.65, inIc:0.85, inOvh:30};
  Object.keys(dflt).forEach(id=>{ const v=+$(id).value;
    if(isFinite(v) && Math.abs(v-dflt[id])>1e-9) parts.push({inMfu:'mfu',inMbu:'mbu',inIc:'ic',inOvh:'ovh'}[id]+'='+v); });
  UC.forEach(u=>{ const f=u.f, ci=+f.selCase, cs=ci>=0? CASES[ci] : null;
    parts.push('uc='+(u.name||ucName(u)));
    parts.push('model='+(f.chkCustom
      ? 'custom:'+[f.cusParams,f.cusActive,f.cusHidden,f.cusLayers,f.cusKvh,f.cusHdim,f.cusCtx].join('/')
      : (MODELS[+f.selModel||0]||{}).name));
    parts.push('quant='+(QUANTS[+f.selWQuant||0]||{}).name);
    parts.push('kv='+(KV_QUANTS[+f.selKQuant||0]||{}).name);
    if(cs) parts.push('preset='+cs.name);
    ...
    if(f.selPolicy==='all' && !(cs&&cs.policy==='all')) parts.push('policy=all');
    if(f.selPolicy!=='all' && cs&&cs.policy==='all') parts.push('policy=running');
    put('ttft', +f.sloTtft, cs&&cs.ttftTarget);
    put('tps',  +f.sloTps,  cs&&cs.tpsTarget);
    put('p95',  +f.sloP95,  cs&&cs.p95Target);
    if(u.concManual) parts.push('conc='+(+f.inConc||1));
    else if(+f.nrmUsers) parts.push('users='+(+f.nrmUsers));
    if(u.isolate) parts.push('isolate=1');
    const kinds=(u.supports||[]).filter(sp=>sp.on!==false).map(sp=>sp.kind);
    const def=(ci>=0? defaultSupports(ci) : []).map(sp=>sp.kind);
    if(kinds.join(',')!==def.join(',')) parts.push('supports='+(kinds.length? kinds.join(',') : 'none'));
  });
  return location.origin+location.pathname+'#p=t:'+parts.join(';').replace(/ /g,'+');
}
/* A share link is a one-shot import: the payload becomes a NEW local project
   (never the sender's id, so it can't clobber existing local work), it is
   saved immediately, and the data is stripped from the address bar. Refresh
   then restores the edited copy from this browser, not the stale snapshot;
   the Share button remains the one explicit way to mint a link. */
function importShared(j){
  setProjId(newProjId());
  applyConfig(j); autoSize(true);
  autosave();
  try{ history.replaceState(null, '', location.pathname); }catch(e){}
  toast('Shared project imported · saved as a copy in this browser');
}
  ...
  try{
    if(h.indexOf('#p=')===0){
      ...
      if(payload.indexOf('z:')===0){
        ...
        if(r.error){ toast(r.error, true); try{ history.replaceState(null,'',location.pathname); }catch(e){} return true; }
        importShared(r.json);
        if(r.notes && r.notes.length) setTimeout(()=>toast('Imported with notes: '+r.notes.join(' · ')), 2200);
        return true;
      }
      else return false;
    ...
    if(q.get('p') && SHARE_API){
      ...
      if(r.ok){ const j=await r.json(); importShared(j); return true; }
    }
    ...
    project:{active:activeUc, usecases:UC.map(u=>({id:u.id, name:u.name||null,
      ...
      results:(()=>{ const prj=computeProject(); const F=prj.fleet;
        return {fits:F.fits, sloMet:F.sloAll,
          servingGpus:F.servG, procuredNodes:F.procW, procuredGpus:F.procG, powerKw:+F.kW.toFixed(1),
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><cols>${cols}</cols><sheetData>${body}</sheetData>${drawing?'<drawing r:id="rId1"/>':''}</worksheet>`;
  ...
  prj.pools.forEach(p=>pr(p.state.model.name, p.state.wq.name,
    p.members.map(mi=>ucName(UC[mi])).join(' + '), p.state.tp,
    p.sliced? `${p.d.replicas}× ${sliceName(p.sliced)} on shared GPUs` : `${p.d.replicas*p.state.tp} dedicated GPUs`,
  ...
  pr(); pr('FLEET');
  [['Active GPUs (pools + shared)',prj.fleet.activeG],['of which shared (sliced replicas + supports)',prj.fleet.supG],
   ['Unassigned GPU slots (node packing)',prj.fleet.spare],['Standby nodes (resilience)',prj.fleet.resilW],
  ...
  buildFleetSites(prj).sites.forEach(s2=>s2.nodes.forEach(n2=>{
    const parts=n2.gpus.map((g2,k)=>{
      ...
      if(g2.type==='sup'||g2.type==='shared') return `GPU${k+1}: `+g2.bin.slices.map(sl=>sl.kind==='pool'? `${prj.pools[sl.pool].state.model.name} slice` : (KIND_LABEL[sl.sub||sl.kind]||sl.kind)+(sl.inst>1?' ×'+sl.inst:'')).join('+');
...
function autoNoteHtml(){
  const an=window.__autoNote; if(!an) return '';
  if(!an.entries) return esc(an.text);
  return `<ul class="as-list">${an.entries.map(e=>
    `<li${e.bad?' class="bad"':''}><b>${esc(e.title)}</b>${e.who?` <span class="who">(${esc(e.who)})</span>`:''} · ${esc(e.facts)}${
...
function autoSizeProject(quiet){
  ...
  const tail=[];
  if(done.shared&&done.shared.gpus) tail.push(`${done.pools.some(p2=>p2.sliced)?'Sliced replicas and supporting':'Supporting'} models pack onto ${done.shared.gpus} shared GPU${done.shared.gpus>1?'s':''}`);
  tail.push(`Fleet: ${done.fleet.servG} serving GPUs on ${done.fleet.servW} node${done.fleet.servW>1?'s':''} · ${done.fleet.procG} procured (${RESIL[done.hw.resil].label})`);
  const text=entries.map(e=>`${e.title}${e.who?` (${e.who})`:''}: ${e.facts}${e.subs.length? ' · '+e.subs.join(' · '):''}.`).join(' ')+' '+tail.join('. ')+'.';
  window.__autoNote={text, entries, tail, at:new Date()};
  render();
...
function autoSize(quiet){
  ...
  UC.forEach(u=>{ u.sliceU=0; u.cards=0; u.cardsKey=''; });
  const s=readState();
  const r=solvePool(s);
  ...
  // Keep the CARD plan here too. Applying only inWorkers re-inflated the pool to
  // whole nodes, so a plan the solver verified at 4 cards was reported at 16
  // replicas: the single-use-case mode described a fleet nobody had solved.
  captureUc();
  { const u=UC[activeUc];
    if(u){ u.cards=r.mode==='sliced'? 0 : (r.cards||0);
      u.cardsKey=u.cards? cardsKey(u, readHw()) : ''; } }
  render();
  ...
  const subs=[];
  subs.push(`one ${s.wq.name} copy is ${fmt(weights)} GB: TP${tp} is the smallest group that holds it with cache room${widened?`, widened from TP${tpFit} for the ${fmt(s.sloTtft)} ms TTFT target`:''}`);
  subs.push(crossed
  ...
  const entries=[{title:`${s.model.name} ${s.wq.name}`,
    facts:`TP${tp} × ${workers} worker${workers>1?'s':''} · batch ${batch} · ${df.active} of ${s.concurrent} calls admitted`, subs}];
  const text=entries.map(e=>`${e.title}: ${e.facts} · ${e.subs.join(' · ')}.`).join(' ');
  window.__autoNote={text, entries, tail:[], at:new Date()};
  render();
...
/* ================= PUBLIC API & BOOT ================= */
window.GPUscale = {compute, gpuFitScan, sloOptScan, sloPrice, readState, serialize, applyConfig, buildXls, buildXlsxBytes, render, autoSize, computeProject, solvePool, sliceProfiles, parseTextShare, textShare, MODELS, GPUS, QUANTS, KV_QUANTS, CASES, SUPPORT,
