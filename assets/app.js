...
const STUDIO_VERSION = '5.21.1', ENGINE_VERSION = 25;
...
function coPack(pools, hw){
  ...
  pools.forEach((p,pi)=>{ if(p.sliced) return;
    ...
    const mem=p.d.total/gpus, duty=coDuty(p), safe=coShareSafe(p), tslack=coTtftSlack(p);
    for(let r=0;r<p.d.replicas;r++) items.push({pool:pi, rep:r, tp, mem, duty, safe, tslack});
  });
    ...
    if(it.safe){
      ...
      for(let b=0; b+it.tp<=limit; b+=it.tp){
        ...
        const ttftOk=block.every(g=>{
          ...
          if(infl(it.duty) > it.tslack) return false;
          return g.slots.every(s=>infl(s.duty) <= s.tslack);
        });
        if(roomy&&allSafe&&!samePool&&ttftOk){ block.forEach((g,k)=>{ g.mem+=it.mem; g.duty+=it.duty;
          g.slots.push({pool:it.pool, rep:it.rep, shard:k, mem:it.mem, duty:it.duty, safe:it.safe, tslack:it.tslack}); });
          placed=true; break; }
      }
    }
...
function renderFleet(prj){
  ...
  { const co=prj.co||{}, lim=[];
    if(co.shares) lim.push(`${co.shares} card${co.shares>1?'s':''} carry more than one model`);
