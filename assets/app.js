...
const STUDIO_VERSION = '4.8.1', ENGINE_VERSION = 23;
...
function buildRecs(s,d,m,g,prelaunch){
  ...
  if(perRepNeed>perRepCap){
    ...
    push('crit','One replica cannot hold the model',
      `Each TP${s.tp} replica spans ${fmt(perRepCap)} GB of VRAM but a single copy of ${m.name} needs ${fmt(perRepNeed)} GB. Adding workers cannot fix this: every ${s.tp}-GPU group is another full copy of the weights. Fixes: TP ≥ ${minTp}${minTp>s.perW?` (crosses the ${s.perW}-GPU NVLink island: use a rack-scale part or set interconnect efficiency 0.6 to 0.7)`:''}${fitQ?`, ${fitQ.name} weights (${fmt(s.params*fitQ.bytes)} GB fits at TP${s.tp})`:''}, a higher-VRAM GPU, or a smaller model. Pipeline parallelism is not modeled here.`);
  }
  if(!d.fits){
