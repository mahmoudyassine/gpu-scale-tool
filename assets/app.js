...
function gpuFitScan(prj){
  ...
  const key=prj.pools.map(p=>[p.state.model.name,p.state.wq.name,Math.round(p.state.concurrent),
    Math.round(p.state.resident),p.state.sloTps,p.state.sloP95,p.state.sloTtft].join('/')).join('|')
    +'#'+prj.hw.perW+'#'+(($('autoUtil')&&$('autoUtil').value)||80);
  ...
  GPUS.forEach(g=>{
    ...
    for(const p of prj.pools){
      let r;
      try{ r=solvePool({...p.state, gpu:g, gpuVram:g.vram, gpuBw:g.bw, gpuTflops:g.tflops}); }
...
function coPack(pools, hw){
  ...
  // why a pool could not take a neighbour: the binding reason, in the pool's words
  const blockers=[];
  pools.forEach((p,pi)=>{ if(p.sliced) return;
    if(!(+p.state.sloTps>0)){ blockers.push(`${p.state.model.name} has no per-user speed target, so there is no share to guarantee it`); return; }
    if(!coShareSafe(p)){ blockers.push(`${p.state.model.name} would miss its P95 at the speed a shared card guarantees`); return; }
    const sl=coTtftSlack(p);
    if(isFinite(sl) && sl<1.3) blockers.push(`${p.state.model.name} has only ${Math.round((sl-1)*100)}% first-token headroom, and a co-tenant needs more`);
  });
...
function buildFleetSites(prj){
  ...
  function gpuList(){
    ...
    ((prj.co&&prj.co.gpus)||[]).forEach(g=>{
      ...
      const slices=tenants.map(pi=>{ const st=pools[pi].state;
        ...
        return {kind:'pool', pool:pi, gb:mem, mu:mem,
          tip:`${st.model.name} (TP${st.tp}) · ${fmt(mem)} GB`}; });
      list.push({type:'shared', mix, bin:{co:true, used:g.mem, slices},
  ...
  function serveNodesAll(copy){
    ...
    for(let base=0; base<list.length || out.length===0; base+=hw.perW){
      ...
      else if(chunk.some(g2=>g2.type==='shared')&&poolsIn.size===0){ node.poolName='shared / supporting'; }
...
function renderFleet(prj){
  ...
  const memStack=(mix, col, op)=>{
    if(!mix) return '';
    const V=Math.max(1,hw.g.vram), pc=x=>Math.max(0,Math.min(100, x/V*100));
    return `<div class="fm-stack" style="opacity:${op==null?1:op}">`
      + `<div class="ms w" style="height:${pc(mix.w)}%;background:${col}"></div>`
      + `<div class="ms kv" style="height:${pc(mix.kv)}%;background:${col}"></div>`
      + `<div class="ms o" style="height:${pc(mix.o)}%"></div></div>`;
  };
  const gpuHtml=g2=>{
    ...
    if(g2.type==='sup'||g2.type==='shared'){ let y=0, memUsed=0, anyLab=false;
      ...
      if(coBin&&g2.mix) return `<div class="fm-gpu assigned" title="${esc(g2.tip)}">${memStack(g2.mix,'var(--seg-kv)')}${gp(Math.min(1,used))}</div>`;
  ...
  const nodeHtml=n2=>{
    const role = n2.cls==='serve'? esc(n2.pool!=null? poolTag(n2.pool) : (n2.poolName||'mixed'))
  ...
  $('fleetList').innerHTML='<h3>Deployment, text form</h3>'+sites.map(s2=>`<h4>${esc(s2.title)}</h4><ul>`+s2.nodes.map(n2=>{
    ...
    const parts=n2.gpus.map((g2,k)=>{
      ...
      if(g2.type==='sup'||g2.type==='shared') return `GPU ${k+1}: `+g2.bin.slices.map(sl=>sl.kind==='pool'? `${pools[sl.pool].state.model.name} replica slice` : `${KIND_LABEL[sl.sub||sl.kind]||sl.kind} ${sl.model||''}${sl.inst>1?' ×'+sl.inst:''}`).join(', ');
...
/* ================= PUBLIC API & BOOT ================= */
window.GPUscale = {compute, gpuFitScan, readState, serialize, applyConfig, buildXls, buildXlsxBytes, render, autoSize, computeProject, solvePool, sliceProfiles, MODELS, GPUS, QUANTS, CASES,
