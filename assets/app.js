...
const STUDIO_VERSION = '5.21.0', ENGINE_VERSION = 25;
...
function ucState(u, hw){
  ...
  // pool may own any number of GPUs. A hand-edited worker count clears this.
  const gpus=Math.max(1, +u.cards>0? +u.cards : workers*hw.perW);
...
function serialize(){
  ...
  return {
    ...
    project:{active:activeUc, usecases:UC.map(u=>({id:u.id, name:u.name||null,
      isolate:!!u.isolate, supports:u.supports||[], concManual:!!u.concManual, sliceU:+u.sliceU||0, cards:+u.cards||0,
...
function applyConfig(raw){
  ...
  if(proj && Array.isArray(proj.usecases) && proj.usecases.length){
    ...
    proj.usecases.forEach(pu=>UC.push({id:'uc'+(++ucSeq), name:pu.name||'',
      supports:Array.isArray(pu.supports)?pu.supports:[], isolate:!!pu.isolate, concManual:!!pu.concManual, sliceU:+pu.sliceU||0, cards:+pu.cards||0, f:{}}));
...
function autoSize(quiet){
  ...
  if(r.grewForSlo) subs.push(`${r.grewForSlo} worker${r.grewForSlo>1?'s':''} added purely for the speed / P95 targets`);
